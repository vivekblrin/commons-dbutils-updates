...
public class BeanProcessor {
	...
	private static final Logger logger = LogManager.getLogger(BeanProcessor.class);
    ...
    private void callSetter(final Object target, final PropertyDescriptor prop, Object value)
        ...
        try {
            ...
            if (!this.isCompatibleType(value, firstParam)) {
            	logger.error("Error message" + "Cannot set " + prop.getName() + ": incompatible types, cannot convert " + value.getClass().getName() + " to " + firstParam.getName());
    ...
    private boolean matchesPrimitive(final Class<?> targetType, final Class<?> valueType) {
        ...
        try {
        	...
        	logger.error("Error message" +ignored.getMessage());
        }
        return false;
    }
